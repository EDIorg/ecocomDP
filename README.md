...
Accessing data from the Environmental Data Initiative (EDI) repository
requires an API key.

### 1. Obtain an EDI API Key

1.  Visit the [EDI Identity and Access Manager
    (IAM)](https://auth.edirepository.org/) and log in (or create a free
    account).
2.  Navigate to the [Access
    Keys](https://auth.edirepository.org/auth/ui/key) page.
...
Add `EDI_API_KEY` to your user-level `.Renviron` file so it is loaded
