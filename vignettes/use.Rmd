...
Accessing datasets from the Environmental Data Initiative (EDI) Data Repository requires an EDI API key. Set the `EDI_API_KEY` environment variable prior to calling search or read functions:

```r
# Persistent (recommended - add to ~/.Renviron and restart R):
...
To obtain a key, visit the [EDI Identity and Access Manager (IAM)](https://auth.edirepository.org/) and navigate to the [Access Keys](https://auth.edirepository.org/auth/ui/key) page.
