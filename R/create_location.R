...
#' @param nesting (character) Columns in \code{L0_wide} containing location names and listed from high to low. Example: \code{nesting = c("block", "plot", "subplot")}.
#' 
#'
#' @details This function collects specified columns from \code{L0_wide}, creates data frames for each \code{nesting}, assigns \code{latitude}, \code{longitude}, and \code{elevation} to the lowest nesting level (i.e. the observation level) returning \code{NA} for higher levels (these will have to be filled manually afterwards), and determines the relationships between location_id and parent_location_id from \code{L0_wide} and \code{nesting}.
#'     
