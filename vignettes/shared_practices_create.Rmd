---
title: "Shared Practices for Creating an ecocomDP Dataset"
output:
  rmarkdown::html_vignette:
    toc: true
    fig_caption: yes
vignette: >
  %\VignetteIndexEntry{Shared Practices for Creating an ecocomDP Dataset}
...
library(ecocomDP)
```
This document assumes you understand the ecocomDP model. To learn about the model and its scope, refer to the [Model Overview vignette](model_overview.html) and the [Ecological Informatics article](https://doi.org/10.1016/j.ecoinf.2021.101374).
...
Early sections address important considerations for anyone thinking about converting source datasets to the ecocomDP model, then focus shifts to examination of the model components in greater detail. These shared practices are written with the intention to help simplify the conversion process.

If you are new to the conversion process, we recommend reading the [Getting Started](#getting_started) and [Concepts](#concepts) sections, reviewing the [Create](create.html) and [Model Overview](https://github.com/EDIorg/ecocomDP/tree/master/model) vignettes, and referring back to this document as questions arise. A thorough understanding of the ecocomDP model and some foundational concepts will greatly simplify the conversion process.
...
```
The Level 0 (L0) dataset is the incoming original data with complete metadata. Level 1 (L1) dataset are the L0 datasets reformatted according to the predefined ecocomDP model. Researchers are able to use L1 as inputs to speed their analyses and generate Level 2 (L2) data. (Adapted from [O’Brien et al. 2021](https://doi.org/10.1016/j.ecoinf.2021.101374)).
...
*   Are about community surveys not population surveys. An [ecological community](http://www.ontobee.org/ontology/PCO?iri=http://purl.obolibrary.org/obo/PCO_0000002) is a multi-species collection of organisms of at least two different species, living in a particular area, whereas a [population](http://purl.obolibrary.org/obo/PCO_0000001) is a collection of organisms, all of the same species, that live in the same place.
...
1. Modify L0 components - Modifying L0 components is only permitted in rare cases. This list highlights the L0 components and specific scenarios in which you should modify them:
    ...
    *   Missing value codes - Missing value codes that are explicitly declared in the metadata should be converted to NA. When not declared but unambiguously interpretable as a missing value (e.g. “” in a comments field) then convert to NA. If it is ambiguous, don’t modify the L0 data.
...
If you encounter tables that can’t be joined to the core observation information, possibly because they focus on a different time/space/taxon entirely, omit these problematic tables completely. Apply the following changes to the conversion script to highlight the table omission:
...
For details on the processes within the conversion script, see the [Create Vignette](create.html).
...
Refer to this section to resolve specific issues while creating the L1 tables. For more in-depth descriptions on these tables and their columns, see the [Model Overview Vignette](model_overview.html).

### observation
Store the core observations being analyzed in this table. Observations must be linked to a taxon and to a location. 

#### event_id

*   If an L0 dataset contains values corresponding to the survey/event number, always use these values. 
*   If the dataset does not contain values corresponding to the survey/event number, group observations by the frequency of survey (see (Frequency of Survey)[#frequency_of_survey] for more information) and programmatically assign an event_id. Note: Always arrange the dataset chronologically before assigning event_id; event_ids should correspond chronologically to surveys.
*   If the frequency of survey can not be determined accurately from the metadata or from patterns in the L0 data, omit the event_id column entirely.

#### datetime

*   If the L0 data tables do not contain a date or datetime column, but instead have start_date and end_date columns, use the date from the start_date column to assign datetime in the observation table. Additionally, preserve start_date and end_date as-is in the observation_ancillary table.

#### taxon_id

*   If variables (columns) in L0 data tables are individual species, gather (e.g. `tidyr::pivot_longer()`) into columns of values and taxon_names, and manually add the variable_name and unit columns to describe the measurement. Use the column description and units from the L0 metadata.
...
*   Every dataset has a core observation. In certain cases, it is acceptable to have multiple core observation variables.
    ...
    *   Core observations (e.g. abundance/counts) that are subdivided into smaller classified groups should constitute their own core observation.
...
*   If an L0 dataset does not contain location identifiers they should be formed by grouping on the location name at the meaningful level of observation (see [Levels of Observation](#levels_of_observation) section).
*   If an L0 dataset does not contain location identifiers or names corresponding to location at the meaningful level of observation and the entire study occurs at the same location, a location_id of 1 and a logical location_name derived from the L0 metadata should be assigned to each observation.
...
*   Each row of the location table should contain a latitude and longitude value describing a point location. This information should be in the L0 dataset (e.g. metadata, data tables, or other entities). Sometimes there is geographic information at a different spatial scale than the listed locations. Follow these recommendations for determining the correct coordinates to provide:
    *   The L0 dataset only has areas/polygons (instead of point locations) at a location. In such cases, use the centroid of each area (Figure 1A).
    *   The L0 dataset only has point locations at finer spatial scale than a location. In such cases, use centroid of the constituent coordinates (e.g. centroid of transects within a site) (Figure 1B)\ref{fig:myfig}.
    *   The L0 dataset only has point locations at a coarser level than a location. In such cases, transpose coarser coordinates to the location (e.g. each plot within a site would get the site’s coordinates) (Figure 1C).
    *   The L0 dataset only has an area/polygon at a coarser level than a location. In such cases, use the centroid of the area/polygon as coordinates for the location (Figure 1D).
    *   The L0 dataset has incomplete but accurate point locations for some locations. In such cases, the one-to-one matching of geographic coverage with some of the locations is best. The L1 user may be able to infer coverage to other adjacent locations with coordinates and elevations having NA values (Figure 1E).
    *   Coordinates of location vary with each observation (e.g. specific coordinates where a plankton tow was done within the boundaries of an established/static station, [edi.338.2](https://portal.edirepository.org/nis/mapbrowse?scope=edi&identifier=338&revision=2)) (Figure 1F).
...
```{r echo=FALSE, out.width='100%', fig.align="center", fig.cap='Figure 1. Mapping coordinates between spatial scales.'}
knitr::include_graphics('./coords_final_wide.png')
```    
...
Store summary info calculated from incoming data in this table.
...
Store variables that pertain directly to observations in this table (plot or transect identifiers, measurement times, environmental conditions, field notes, etc.).
...
Store variables that pertain directly to the locations in the location table in this table (treatments, habitats, etc.).
...
Store taxon information that pertains to the core taxa in this table.
...
*   Maintain an ongoing list of “considerations” for improving the L0 dataset to share with the L0 author/manager. These are items that did not necessarily hamper production of the L1 dataset but may improve the source dataset if addressed.
...
A dataset often has multiple levels of observation (i.e. spatial scales of observation). Only one of those levels of observation is considered meaningful. We use “meaningful” to denote the finest spatial scale at which the observations are meant to be interpreted.
...
*   If there are multiple levels of observation, only one should be deemed meaningful. Consider the following:
    ...
    *   If not explicitly stated in the L0 metadata, determine if there are changing habitats, experimental treatments, or elevations across any levels of observation, as demonstrated in [Figure 2.](#fig2) 
...
*   If a habitat or other special variable corresponds to a physical location identifier (plot, subplot, transect, etc.), use the physical location identifier to group locations.
    *   The habitat or special variable then belongs in location_ancillary.

### Determining the Meaningful Level of Observation (Visual Examples) {#fig2}
```{r echo=FALSE, out.width='90%', fig.align='center', fig.cap = 'Figure 2. Varying experimental conditions change the meaningful level of observation in a study.'}
knitr::include_graphics('./MLO_final_long.png')
```    
...
*   If the Frequency of Survey can not be determined from the L0 metadata, search for patterns in the L0 data:
    ...
    *   Graphing locations over time:
        *   A visual aid can help you identify trickier patterns such as seasonal samplings or surveys that are roughly monthly but start and end in different months. [Figure 3.](#fig3) demonstrates a variety of sampling patterns that may arise at each Frequency of Survey.
    *   If you can not identify a single Frequency of Survey that is consistent throughout the entire study, attempt to determine the Frequency of Survey for each year of the study.
        *   A study may start at one sampling frequency but change once or more over time. A study that starts as an  annual survey could change to seasonal or monthly over time.
    *   It is possible that the Frequency of Survey is not explicitly stated in the L0 metadata and can not be determined patterns in the L0 data.
        *   If this is the case, you should omit the event_id column entirely.

### Frequency of Survey (Visual Examples) {#fig3}
```{r echo=FALSE, out.width='90%', fig.align='center', fig.cap = 'Figure 3. Instances of Frequency of Survey on a timeline; Using Frequency of Survey to assign event_id.'}
knitr::include_graphics('./LevelOfSurvey_final_long.png')
```

## Value-Added Features {#value_added_features}

Value added information is anything that an ecocomDP creator intentionally adds to the L1 dataset to improve its findability, accessibility, interoperability, or reusability (see [FAIR Principles](https://www.go-fair.org/fair-principles/)). This information does not come from the L0 dataset directly but rather is derived from it. Since the rule of thumb regarding L1 creation is to rearrange but not alter the L0 dataset, supplementing a dataset with value added information must be done carefully and within the constraints of the following rules:

*   Taxonomic coverage - Resolve taxa to an authority system and list in the taxon table only when the L0 does not resolve taxa to an authority system.
...
Sometimes an L1 needs to be removed from circulation. To do this for an archived L1, publish an identical revision of the dataset with the following changes added manually to the metadata:

*   Add "deprecated" (lower case) to the list of keywords. 
*   Prefix the dataset title with: "[DEPRECATED]". 
*   Provide rationale as a new and first <para> of the dataset abstract.
