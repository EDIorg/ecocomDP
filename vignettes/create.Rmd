...
> **Note**: Reading source datasets or metadata from the EDI Data Repository requires an EDI API key. Configure the `EDI_API_KEY` environment variable in your `.Renviron` or active R session (obtainable from the [EDI Identity and Access Manager Access Keys](https://auth.edirepository.org/auth/ui/key) page).
